...
on 5xx. Cost model for a mid-sized account (tens of invoices, ~140 requests): the
per-invoice message+payment loops dominate at 2 calls each — minutes, not hours.
Print a call-count estimate
before starting.
