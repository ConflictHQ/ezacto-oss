...
on 5xx. Cost model for a small account (~57 invoices): the per-invoice message+payment
...
| **Negative time entries.** Harvest corrects an over-logged timesheet with a negative entry offsetting an earlier one (the source account had several, one of them recent and unlocked). | **Imported.** `time_entries.seconds`, `seconds_without_timer` and `rounded_seconds` were `CHECK (… BETWEEN 0 AND …)` and are now `abs(…) <= limit`, the same signed shape `invoice_payments.amount_cents` took. Skipping was never safe: Harvest nets these entries out and ezacto could not, so a month's uninvoiced total read high against Harvest's by the exact size of the correction — an overpayment to one contractor on a live payroll run. Budgets and retainer balances keep their non-negative CHECK, because a budget really is a magnitude. Not a gap. |
| **Non-positive invoice payments.** Harvest records $0 payments settling $0 invoices, and negative payments settling credit notes (the source account had several of the former and one sizeable negative). | **Imported.** `invoice_payments.amount_cents` was `CHECK (… BETWEEN 1 AND …)` and is now `abs(...) <= limit`, the same signed shape `invoices.amount_cents` has always had; `bank_deposits` keeps the strict rule, because a bank credit really is positive. Skipping was never safe for this class: invoice state is derived from imported payments (`packages/db/src/internal/invoice-import.ts`), so an invoice left with no payments landed `open` and all seven affected invoices flipped from `paid`. Not a gap. |
