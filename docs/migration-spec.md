...
| **Non-positive invoice payments.** Harvest records $0 payments settling $0 invoices, and negative payments settling credit notes (CONFLICT has six of the former and one of -$8,765). `invoice_payments.amount_cents` is `CHECK (… BETWEEN 1 AND …)` — a strictly positive receipt. | Skipped, each recorded as a `non_positive_payment` anomaly carrying the invoice and source amount. **The invoice does not keep the `state` Harvest gave it.** State is derived from imported payments (`packages/db/src/internal/invoice-import.ts:1344-1352`), so an invoice left with no payments lands `open`; all seven of CONFLICT's flip from `paid` to `open`. The loader raises a `source_state_disagrees` diagnostic for exactly this and `packages/migrate/src/load.ts` currently discards it. Skipping is therefore **not** a safe handling for this class — it silently restates settled invoices as unpaid — and the CHECK needs widening rather than the payment dropping. |
