...
afterEach(async () => {
  await Promise.all(miniflares.splice(0).map(async (miniflare) => miniflare.dispose()))
})
...
describe('Worker operator bootstrap', () => {
  it('[e2e:cli-log] seeds once, rejects mismatch, authenticates whoami, and never returns the bearer', async () => {
    ...
    const unauthorized = await request('/__ezacto/bootstrap', post(wrongToken, identity))
    responses.push(await unauthorized.text())
    expect(unauthorized.status).toBe(401)
    expect(unauthorized.headers.get('www-authenticate')).toBe('Bearer realm="ezacto-bootstrap"')
    ...
    const seeded = await request('/__ezacto/bootstrap', post(bootstrapToken, identity))
    ...
    const exactRetry = await request('/__ezacto/bootstrap', post(bootstrapToken, identity))
    ...
    expect(
      await database
        ...
        .first<{ id: string }>(),
    ).toEqual({ id: '0017_email_delivery_details' })
  }, 20_000)

  it('[security] remains unavailable when the temporary Worker secret is absent', async () => {
    const { request } = await harness(false)
    const response = await request('/__ezacto/bootstrap', post(bootstrapToken, identity))
