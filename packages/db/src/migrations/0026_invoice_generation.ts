import { estimateCommandsMigration } from './0021_estimate_commands.js'

const historicalEventOutboxInsertGuard =
  estimateCommandsMigration[estimateCommandsMigration.length - 1]!

const creationAuthorityStart = `            OR NOT EXISTS (
              SELECT 1 FROM estimate_command_ledger command`
const extendedCreationAuthorityStart = `            OR (
              NOT EXISTS (
                SELECT 1 FROM estimate_command_ledger command`
const creationAuthorityEnd = `                AND json_extract(NEW.payload_json, '$.invoice.after.payment_status') = 'unpaid'
            )
          ) THEN RAISE(ABORT, 'invoice creation event requires its pending estimate command')`
const extendedCreationAuthorityEnd = `                AND json_extract(NEW.payload_json, '$.invoice.after.payment_status') = 'unpaid'
              )
              AND NOT EXISTS (
          ...
          ) THEN RAISE(ABORT, 'invoice creation event requires its pending create command')`

const occurrences = (source: string, needle: string): number => source.split(needle).length - 1

if (
  occurrences(historicalEventOutboxInsertGuard, creationAuthorityStart) !== 1 ||
  occurrences(historicalEventOutboxInsertGuard, creationAuthorityEnd) !== 1
) {
  throw new Error('invoice generation migration found an unexpected historical event guard')
}

const eventOutboxInsertGuardMigration = historicalEventOutboxInsertGuard
  .replace(creationAuthorityStart, extendedCreationAuthorityStart)
  .replace(creationAuthorityEnd, extendedCreationAuthorityEnd)

if (eventOutboxInsertGuardMigration === historicalEventOutboxInsertGuard) {
  throw new Error('invoice generation migration could not extend invoice.created authority')
}

const invoiceGenerationEventGuard = `CREATE TRIGGER invoice_generation_event_outbox_guard
  ...
  BEGIN
    SELECT CASE WHEN (
      ...
      OR NOT EXISTS (
        ...
        WHERE command.invoice_id = NEW.aggregate_id
          ...
          AND json_extract(NEW.payload_json, '$.trigger.type') = 'invoice_header'
          AND json_extract(NEW.payload_json, '$.trigger.id') = invoice.id
          ...
          AND json_extract(command.request_json, '$.from') = invoice.period_start
          AND json_extract(command.request_json, '$.to') = invoice.period_end
    ...
    ) THEN RAISE(ABORT, 'invoice creation event does not match its pending command') END;
  END`

const generationCommandMatch = `command.invoice_id = NEW.aggregate_id
        AND command.command_id = NEW.command_id
        AND command.command_kind = 'invoice.create' AND command.completed = 0`

const invoiceGenerationLineManifestGuard = `CREATE TRIGGER invoice_generation_line_manifest_guard
  BEFORE INSERT ON event_outbox
  WHEN NEW.aggregate_type = 'invoice' AND NEW.event_type = 'invoice.created'
  BEGIN
    SELECT CASE WHEN EXISTS (
      SELECT 1 FROM invoice_command_ledger command
      WHERE ${generationCommandMatch} AND (
        ...
        OR EXISTS (
          ...
          WHERE NOT EXISTS (
            ...
            WHERE line.invoice_id = NEW.aggregate_id
              ...
              AND line.description = json_extract(member.value, '$.description')
              ...
              AND line.project_id = json_extract(member.value, '$.project_id')
          )
        )
      )
    ) THEN RAISE(ABORT, 'invoice creation lines do not match their manifest') END;
  END`

const invoiceGenerationTimeManifestGuard = `CREATE TRIGGER invoice_generation_time_manifest_guard
  BEFORE INSERT ON event_outbox
  WHEN NEW.aggregate_type = 'invoice' AND NEW.event_type = 'invoice.created'
  BEGIN
    SELECT CASE WHEN EXISTS (
      SELECT 1 FROM invoice_command_ledger command
      WHERE ${generationCommandMatch} AND (
        json_extract(command.request_json, '$.time_entry_count') <> (
          SELECT count(*) FROM time_entries entry WHERE entry.invoice_id = NEW.aggregate_id
        )
        OR json_extract(command.request_json, '$.time_entry_count')
          <> json_array_length(command.source_manifest_json, '$.time_entries')
        OR EXISTS (
          SELECT 1 FROM json_each(command.source_manifest_json, '$.time_entries') member
          WHERE NOT EXISTS (
            ...
            WHERE entry.invoice_id = NEW.aggregate_id
              AND entry.id = json_extract(member.value, '$.id')
              AND entry.project_id = json_extract(member.value, '$.project_id')
              AND project.name = json_extract(member.value, '$.project_name')
              AND entry.task_id = json_extract(member.value, '$.task_id')
              AND task.name = json_extract(member.value, '$.task_name')
              AND entry.user_id = json_extract(member.value, '$.user_id')
              AND trim(user.first_name || ' ' || coalesce(user.last_name, ''))
                = json_extract(member.value, '$.user_name')
              AND entry.spent_date = json_extract(member.value, '$.spent_date')
              AND entry.notes IS json_extract(member.value, '$.notes')
              AND upper(coalesce(project.billing_currency, client.currency))
                = json_extract(member.value, '$.currency')
              AND entry.rounded_seconds = json_extract(member.value, '$.rounded_seconds')
              AND entry.billable_rate_cents IS json_extract(member.value, '$.billable_rate_cents')
              AND entry.updated_at = json_extract(member.value, '$.updated_at')
              AND entry.billable = 1 AND entry.timer_started_at IS NULL
              AND NOT (entry.started_time IS NOT NULL AND entry.ended_time IS NULL)
          )
        )
      )
    ) THEN RAISE(ABORT, 'invoice creation time entries do not match their manifest') END;
  END`

const invoiceGenerationExpenseManifestGuard = `CREATE TRIGGER invoice_generation_expense_manifest_guard
  BEFORE INSERT ON event_outbox
  WHEN NEW.aggregate_type = 'invoice' AND NEW.event_type = 'invoice.created'
  BEGIN
    SELECT CASE WHEN EXISTS (
      SELECT 1 FROM invoice_command_ledger command
      WHERE ${generationCommandMatch} AND (
        json_extract(command.request_json, '$.expense_count') <> (
          SELECT count(*) FROM expenses expense WHERE expense.invoice_id = NEW.aggregate_id
        )
        OR json_extract(command.request_json, '$.expense_count')
          <> json_array_length(command.source_manifest_json, '$.expenses')
        OR EXISTS (
          SELECT 1 FROM json_each(command.source_manifest_json, '$.expenses') member
          WHERE NOT EXISTS (
            ...
            WHERE expense.invoice_id = NEW.aggregate_id
              AND expense.id = json_extract(member.value, '$.id')
              AND expense.project_id = json_extract(member.value, '$.project_id')
              AND project.name = json_extract(member.value, '$.project_name')
              AND expense.expense_category_id = json_extract(member.value, '$.category_id')
              AND category.name = json_extract(member.value, '$.category_name')
              AND expense.user_id = json_extract(member.value, '$.user_id')
              AND trim(user.first_name || ' ' || coalesce(user.last_name, ''))
                = json_extract(member.value, '$.user_name')
              AND expense.spent_date = json_extract(member.value, '$.spent_date')
              AND expense.notes IS json_extract(member.value, '$.notes')
              AND expense.units IS json_extract(member.value, '$.units')
              AND upper(coalesce(project.billing_currency, client.currency))
                = json_extract(member.value, '$.currency')
              AND expense.total_cost_cents = json_extract(member.value, '$.total_cost_cents')
              AND expense.updated_at = json_extract(member.value, '$.updated_at')
              AND expense.billable = 1
          )
        )
      )
    ) THEN RAISE(ABORT, 'invoice creation expenses do not match their manifest') END;
  END`
...
/** Extends D22's durable command vocabulary with its reserved creation command. */
export const invoiceGenerationMigration = [
  ...
  `ALTER TABLE invoice_command_ledger RENAME TO invoice_command_ledger_0025`,
  `CREATE TABLE invoice_command_ledger (
    ...
    CHECK ((command_kind = 'invoice.create' AND COALESCE((
      ...
      OR (command_kind <> 'invoice.create' AND request_json IS NULL
  ...
  `INSERT INTO invoice_command_ledger (
    ...
    expected_invoice_version, occurred_at, event_count, completed,
    first_aggregate_sequence, result_json, completed_at
  ) SELECT invoice_id, command_id, command_kind, input_fingerprint, actor_type, actor_id,
    expected_invoice_version, occurred_at, event_count, completed,
    first_aggregate_sequence, result_json, completed_at
  FROM invoice_command_ledger_0025`,
  `DROP TABLE invoice_command_ledger_0025`,
  ...
  `CREATE TRIGGER invoice_command_ledger_insert_guard
    ...
    BEGIN
      SELECT CASE
        ...
        WHEN NEW.command_kind = 'invoice.create'
        ...
        WHEN NEW.command_kind NOT IN ('invoice.create','invoice.view') AND NOT EXISTS (
  ...
  `CREATE TABLE invoice_number_sequence (
    ...
    next_number INTEGER NOT NULL CHECK (next_number BETWEEN 1 AND 9007199254740991)
  ) STRICT`,
  `INSERT INTO invoice_number_sequence (singleton, next_number)
    SELECT 1, COALESCE(MAX(CAST(number AS INTEGER)), 0) + 1
    FROM invoices
    WHERE number = CAST(CAST(number AS INTEGER) AS TEXT)
      AND CAST(number AS INTEGER) BETWEEN 0 AND 9007199254740990`,
  `CREATE TRIGGER invoice_number_sequence_insert_advance
    AFTER INSERT ON invoices
    WHEN NEW.number = CAST(CAST(NEW.number AS INTEGER) AS TEXT)
      AND CAST(NEW.number AS INTEGER) BETWEEN 0 AND 9007199254740990
    BEGIN
      UPDATE invoice_number_sequence
      SET next_number = max(next_number, CAST(NEW.number AS INTEGER) + 1)
      WHERE singleton = 1;
    END`,
  `CREATE TRIGGER invoice_number_sequence_update_advance
    AFTER UPDATE OF number ON invoices
    WHEN NEW.number = CAST(CAST(NEW.number AS INTEGER) AS TEXT)
      AND CAST(NEW.number AS INTEGER) BETWEEN 0 AND 9007199254740990
    BEGIN
      UPDATE invoice_number_sequence
  ...
  invoiceGenerationEventGuard,
  invoiceGenerationLineManifestGuard,
  invoiceGenerationTimeManifestGuard,
  invoiceGenerationExpenseManifestGuard,
] as const
