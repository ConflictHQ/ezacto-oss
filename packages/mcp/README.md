...
project budgets, `reports:read` for organization reports, and `clients:read`
when a report identifies a client by name. The API remains the authority for
row access and profile-based redaction.
...
`list_reports` exposes the same report inventory as the web app and CLI;
`run_report` runs any listed report with its supported filters. The focused
`get_uninvoiced`, `get_client_rollup`, and project-budget tools remain available
for concise prompts and backward compatibility.
