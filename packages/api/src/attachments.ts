...
export interface AttachmentMetadataPort {
  createInvoiceAttachment(input: AttachmentMetadataInput & { invoiceId: number }): Promise<AttachmentRecord>
  createRecurringInvoiceAttachment(
    input: AttachmentMetadataInput & { recurringInvoiceId: number },
  ): Promise<AttachmentRecord>
  createEstimateAttachment(input: AttachmentMetadataInput & { estimateId: number }): Promise<AttachmentRecord>
  createExpenseAttachment(input: AttachmentMetadataInput & { expenseId: number }): Promise<AttachmentRecord>
  createProjectAttachment(input: AttachmentMetadataInput & { projectId: number }): Promise<AttachmentRecord>
...
type OwnerDefinition = {
  owner: AttachmentOwnerType
  collection: 'invoices' | 'recurring-invoices' | 'estimates' | 'expenses' | 'projects'
  singular: string
  parameter: 'invoiceId' | 'recurringInvoiceId' | 'estimateId' | 'expenseId' | 'projectId'
  ...
  list(metadata: AttachmentMetadataPort, parentId: number): Promise<readonly AttachmentRecord[]>
  get(
    metadata: AttachmentMetadataPort,
    parentId: number,
    attachmentId: number,
...
const owners: readonly OwnerDefinition[] = [
  {
    ...
    parameter: 'invoiceId',
    scope: 'invoices',
    create: (metadata, invoiceId, input) => metadata.createInvoiceAttachment({ ...input, invoiceId }),
  ...
  },
  {
    ...
    create: (metadata, recurringInvoiceId, input) =>
      metadata.createRecurringInvoiceAttachment({ ...input, recurringInvoiceId }),
  ...
  },
  {
    ...
    scope: 'invoices',
    create: (metadata, estimateId, input) => metadata.createEstimateAttachment({ ...input, estimateId }),
    list: (metadata, estimateId) => metadata.listEstimateAttachments(estimateId),
  ...
  },
  {
    ...
    parameter: 'expenseId',
    scope: 'expenses',
    create: (metadata, expenseId, input) => metadata.createExpenseAttachment({ ...input, expenseId }),
  ...
  },
  {
    ...
    parameter: 'projectId',
    scope: 'projects',
    create: (metadata, projectId, input) => metadata.createProjectAttachment({ ...input, projectId }),
...
const requireOptions = (options: AttachmentRouteOptions | undefined): AttachmentRouteOptions =>
  options ?? unavailable()
...
const requireOwnerAccess = async (
  ...
  if (await options.authorizeOwnerAccess({ owner: owner.owner, parentId, access, principal })) {
    return
  }
  throw new ApiError({
    status: 404,
...
const attachmentEnvelope = (owner: OwnerDefinition, parentId: number, record: AttachmentRecord) => ({
...
const fileName = (value: string): string => {
  if (value.length < 1 || value.length > 255 || !value.trim() || hasControlCharacter(value)) {
    throw validationError([
      { field: 'file.name', code: 'invalid', message: 'file name must be 1-255 visible characters' },
    ])
  }
  return value
}
...
const mediaType = (value: string): string => {
  ...
  if (normalized.length > 255 || hasControlCharacter(normalized)) {
    throw validationError([
      { field: 'file.type', code: 'invalid', message: 'file content type is invalid' },
    ])
  }
  return normalized
}
...
const multipartFile = async <Bindings extends object>(
  ...
  if (!/^multipart\/form-data\s*;/i.test(contentType) || !/boundary\s*=\s*(?:"[^"]+"|[^;\s]+)/i.test(contentType)) {
    throw new ApiError({
      status: 415,
      code: 'unsupported_media_type',
      message: 'Attachment uploads require multipart/form-data with a boundary.',
    })
  }
  let form: FormData
  try {
  ...
  if (entries.length !== 1 || entries[0]?.[0] !== 'file') {
    throw validationError([
      { field: 'file', code: 'exactly_one_required', message: 'provide exactly one file part and no other fields' },
    ])
  }
  const value = entries[0][1]
  if (!(value instanceof Blob) || typeof (value as File).name !== 'string') {
    throw validationError([{ field: 'file', code: 'file_required', message: 'file must be a binary file part' }])
  }
...
const translateMetadataError = (error: unknown): never => {
  ...
  if (error instanceof TypeError || error instanceof RangeError) {
    throw validationError([
      { field: 'file', code: 'invalid', message: 'The attachment metadata is invalid.' },
    ])
  }
  if (error instanceof Error && /foreign key constraint failed|does not exist/i.test(error.message)) {
    throw new ApiError({ status: 404, code: 'not_found', message: 'The attachment owner does not exist.' })
  }
  if (
    error instanceof Error &&
    /(unique constraint|content identity|file object|attachment identity|constraint failed)/i.test(error.message)
  ) {
    throw new ApiError({
      status: 409,
...
export const installAttachmentRoutes = <Bindings extends object>(
  ...
  const noStore: MiddlewareHandler<ApiContext<Bindings>> = async (context, next) => {
    await next()
  ...
  for (const owner of owners) {
    ...
    api.post(collection, async (rawContext) => {
      ...
      const parentId = resourceId(context.req.param(owner.parameter) ?? '', owner.singular)
      await requireOwnerAccess(options, owner, parentId, 'write', context.get('principal'))
    ...
    api.get(collection, async (rawContext) => {
      ...
      const options = requireOptions(supplied)
      const parentId = resourceId(context.req.param(owner.parameter) ?? '', owner.singular)
      await requireOwnerAccess(options, owner, parentId, 'read', context.get('principal'))
      return context.json({
    ...
    api.get(`${collection}/:attachmentId`, async (rawContext) => {
      ...
      const parentId = resourceId(context.req.param(owner.parameter) ?? '', owner.singular)
      await requireOwnerAccess(options, owner, parentId, 'read', context.get('principal'))
      const attachmentId = resourceId(context.req.param('attachmentId') ?? '', 'attachment')
      const record = await owner.get(options.metadata, parentId, attachmentId)
      if (record === null) {
        throw new ApiError({ status: 404, code: 'not_found', message: 'The attachment does not exist.' })
      }
