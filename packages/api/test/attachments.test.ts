...
type Owner = 'invoice' | 'recurringInvoice' | 'estimate' | 'expense' | 'project'
...
const fakeMetadata = () => {
  ...
  const get = async (owner: Owner, parentId: number, attachmentId: number) =>
    (await list(owner, parentId)).find((record) => record.id === attachmentId) ?? null

  const port: AttachmentMetadataPort = {
    createInvoiceAttachment: (input) => create('invoice', input.invoiceId, input),
    createRecurringInvoiceAttachment: (input) =>
      create('recurringInvoice', input.recurringInvoiceId, input),
    createEstimateAttachment: (input) => create('estimate', input.estimateId, input),
    createExpenseAttachment: (input) => create('expense', input.expenseId, input),
    createProjectAttachment: (input) => create('project', input.projectId, input),
    ...
    getInvoiceAttachment: (id, attachmentId) => get('invoice', id, attachmentId),
    getRecurringInvoiceAttachment: (id, attachmentId) =>
      get('recurringInvoice', id, attachmentId),
    getEstimateAttachment: (id, attachmentId) => get('estimate', id, attachmentId),
    getExpenseAttachment: (id, attachmentId) => get('expense', id, attachmentId),
    getProjectAttachment: (id, attachmentId) => get('project', id, attachmentId),
  }
  return {
    port,
    records,
...
const harness = (
  scopes = ownerRoutes.flatMap(({ scope }) => [`${scope}:read`, `${scope}:write`]),
  authorizeOwnerAccess: (input: AttachmentOwnerAccessInput) => Promise<boolean> = async () => true,
...
const upload = (request: ReturnType<typeof harness>['request'], route: string, body: FormData) =>
  request(`/api/v1/${route}/7/attachments`, { method: 'POST', body })
...
describe('owner-scoped attachment routes', () => {
  for (const { owner, route } of ownerRoutes) {
    it(`[unit] uploads, lists, and downloads ${owner} attachments`, async () => {
      ...
      form.set('file', new File(['owner scoped bytes'], `${owner}.txt`, { type: 'text/plain' }))

      const created = await upload(runtime.request, route, form)
      expect(created.status).toBe(201)
      expect(created.headers.get('cache-control')).toBe('no-store')
      const body = await created.json()
      const expectedHash = createHash('sha256').update('owner scoped bytes').digest('hex')
      ...
      expect(await listed.json()).toMatchObject({ data: [{ id: 1, name: `${owner}.txt` }] })

      const metadata = await runtime.request(`/api/v1/${route}/7/attachments/1`)
      expect(metadata.status).toBe(200)
      expect(await metadata.json()).toMatchObject({ data: { id: 1, name: `${owner}.txt` } })

      const downloaded = await runtime.request(`/api/v1/${route}/7/attachments/1/content`)
      ...
      const crossParent = await runtime.request(`/api/v1/${route}/8/attachments/1/content`)
      expect(crossParent.status).toBe(404)
      expect(await crossParent.json()).toMatchObject({ error: { code: 'not_found' } })
    })
  }
  ...
  it('[unit] converges repeated bytes on one content-addressed object key', async () => {
    ...
    for (const name of ['first.txt', 'second.txt']) {
      ...
      expect((await upload(runtime.request, 'invoices', form)).status).toBe(201)
    }
  ...
  it('[security] checks expense ownership and project assignment before metadata or objects', async () => {
    const runtime = harness(
      ...
      async ({ owner, parentId, principal }) =>
        principal.userId === 9 &&
        ((owner === 'expense' && parentId === 7) || (owner === 'project' && parentId === 7)),
      'member',
    )
    const foreignExpense = await runtime.request('/api/v1/expenses/8/attachments')
    expect(foreignExpense.status).toBe(404)
    expect(await foreignExpense.json()).toMatchObject({ error: { code: 'not_found' } })

    const unassignedProject = await runtime.request('/api/v1/projects/8/attachments')
    expect(unassignedProject.status).toBe(404)
    expect(await unassignedProject.json()).toMatchObject({ error: { code: 'not_found' } })

    const form = new FormData()
    form.set('file', new File(['must not persist'], 'denied.txt'))
    const deniedExpenseUpload = await runtime.request('/api/v1/expenses/8/attachments', {
      method: 'POST',
      body: form,
    })
    expect(deniedExpenseUpload.status).toBe(404)
    expect(await deniedExpenseUpload.json()).toMatchObject({ error: { code: 'not_found' } })
  ...
  it('[unit] exposes no generic attachment owner route', async () => {
    const response = await harness().request('/api/v1/attachments?owner_type=invoice&owner_id=7')
